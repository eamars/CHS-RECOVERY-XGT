...
char* INSTALL_MENU_ITEMS[] = {  "从SD卡选择固件",
...
{
    static char* headers[] = {  "从SD卡安装zip刷机包",
                                "",
                                NULL
    };
/* DooMLoRD: FIX for SEMC devices */
...
void show_nandroid_restore_menu(const char* path)
{
    ...
    if (confirm_selection("确认恢复？", "是的，确认恢复"))
...
void show_mount_usb_storage_menu()
{
    ...
    if ((fd = open(BOARD_UMS_LUNFILE, O_WRONLY)) < 0) {
        LOGE("无法打开ums lunfile (%s)", strerror(errno));
        return -1;
    }

    if ((write(fd, vol->device, strlen(vol->device)) < 0) &&
        (!vol->device2 || (write(fd, vol->device, strlen(vol->device2)) < 0))) {
        LOGE("无法写入ums lunfile (%s)", strerror(errno));
        close(fd);
        return -1;
    }
...
int format_unknown_device(const char *device, const char* path, const char *fs_type)
{
    ...
    if (0 == strcmp(path, "/sd-ext"))
    {
        ...
        {
            ui_print("没有发现app2sd分区，跳过格式化/sd-ext.\n");
            return 0;
        }
    }
...
void show_nandroid_advanced_restore_menu(const char* path)
{
    ...
    static char* advancedheaders[] = {  "选择需要恢复的备份",
                                "",
                                "",
                                NULL
    };
...
/* DooMLoRD: FIX for SEMC devices */
    ...
    static char* list[] = { "恢复系统分区 [system]",
                            "恢复恢复用户数据 [data]",
                            "恢复恢复缓存 [cache]",
                            "恢复恢复app2sd [sd-ext]",
                            "恢复wimax [wimax]",
    ...
    {
        case 0:
            if (confirm_selection(confirm_restore, "是的，恢复系统分区 [system]"))
                nandroid_restore(file, 0, 1, 0, 0, 0, 0);
            break;
        case 1:
            if (confirm_selection(confirm_restore, "是的，恢复用户数据 [data]"))
                nandroid_restore(file, 0, 0, 1, 0, 0, 0);
            break;
        case 2:
            if (confirm_selection(confirm_restore, "是的，恢复缓存 [cache]"))
                nandroid_restore(file, 0, 0, 0, 1, 0, 0);
            break;
        case 3:
            if (confirm_selection(confirm_restore, "是的，恢复app2sd [sd-ext]"))
                nandroid_restore(file, 0, 0, 0, 0, 1, 0);
            break;
        case 4:
            if (confirm_selection(confirm_restore, "是的，恢复wimax [wimax]"))
...
*/
    ...
    switch (chosen_item)
    {
        case 0:
            {
		if (confirm_selection("确认备份？", "是的，开始备份！"))
		{                
...
/*
		...
		printline="";
*/
                if (confirm_selection("危险！即将清空SD卡!", "确认，清空SD卡"))
                {
		...
		static char* ext_sizes[] = { "128M",
                ...
                ui_print("正在清空SD卡，请稍等...\n");
                if (0 == __system(cmd))
                    ui_print("Done!\n");
                else
                    ui_print("错误！请查看 /tmp/recovery.log\n");
